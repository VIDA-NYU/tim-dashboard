...
const RecordingCard = (props) => {
    ...
    const handleChangeSelectedRecording = (value) => {
        // Here, we invoke the callback with the new value
        props.onChangeSelectRecording(value);
    }
    ...
    return (
      <Card sx={{ maxWidth: '98%', display: 'flex', flexDirection: 'column' }}>
        ...
        <CardContent sx={{ flexGrow: 1, px: 2, py: 0.5, pt: 0.2 }}>
          <Button variant="outlined" onClick={() => handleChangeSelectedRecording(recording.name) } > 
            {recording.name}
            <ArrowForwardIosIcon />
          </Button>
...
const SessionListView = ({ sortby='first-entry', ...props }) => {
    ...
    const handleChangeSelectedRecording = (newValue) => {
      setValue(newValue);
      props.onChangeSelectRecording(newValue);
    }
